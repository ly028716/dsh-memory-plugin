...
function createImportHtml(data) {
...
<body>
    <h1>正在导入数据...</h1>
    <script>
        // 设置 localStorage；数据已编码，避免用户内容结束当前 script 标签
        localStorage.setItem('memory-plugin-data', ${serializedData});
...
function main() {
    ...
    const htmlCode = createImportHtml(data);
...
module.exports = { createImportHtml, serializeForScript };
